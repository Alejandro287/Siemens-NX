########################## TCL Event Handlers ##########################
#
#  HARTROL_3EJES_MITSUBIHI_700.tcl - 3_axis_mill
#
#    This is a 3-Axis Milling Machine.
#
#  Created by Aio @ Tuesday, December 27, 2016 4:26:59 PM SA Pacific Standard Time
...
#
  ...
  set mom_sys_alt_unit_post_name                "HARTROL_3EJES_MITSUBIHI_700__IN.pui"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_tool_number_max                   "21"
...
#=============================================================
  ...
  # Accumulated time should be in minutes.
   ...
   MOM_force Once G_adjust G_motion
...
#=============================================================
  ...
  global mom_motion_event
   ...
   set rapid_traverse_blk {G_motion G_mode X Y Z M_spindle M_coolant S}
   set rapid_traverse_xy_blk {G_motion G_mode X Y M_spindle M_coolant S}
   set rapid_traverse_yz_blk {G_motion G_mode Y Z M_spindle M_coolant S}
   set rapid_traverse_xz_blk {G_motion G_mode X Z M_spindle M_coolant S}
...
#=============================================================
   ...
   MOM_force Once G_cutcom G_plane G_adjust G_motion G
...
#=============================================================
proc PB_CMD_Automatic_tool_change_set_1_to_0 { } {
#=============================================================
global mom_tool_number


  if  { [expr $mom_tool_number > 01 ]} {
...
#=============================================================
proc PB_CMD__check_block_end_of_path_3 { } {
#=============================================================
...
#=============================================================
proc PB_CMD__check_block_force_tool_output { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
...
#=============================================================
proc PB_CMD__check_block_rapid_spindle { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
...
# This custom command is for tool_change
        ...
        MOM_set_seq_off
        MOM_output_literal "$mom_sys_control_out $mom_tool_name Tool Number $mom_tool_number DIA.OFF. - $mom_tool_number LEN. - $mom_tool_number Tool Diameter. [format "%5.3f" $mom_tool_diameter]. $mom_sys_control_in"
        MOM_set_seq_on
...
if {[info exists mom_next_oper_has_tool_change] && [info exists first_tool_change]} {
...
proc PB_CMD_footer { } {
#=============================================================
global mom_machine_time mom_cutting_time ci co

set hours [format %2.0f [expr [format %2.0f $mom_machine_time] / 60]]
set minutes [format %2.0f [expr $mom_machine_time - 60 * $hours]]

if { $hours > 0} {
MOM_output_literal "$co TIEMPO TOTAL:$hours Hrs.$minutes min.$ci"
   } else {
MOM_output_literal "$co TIEMPO TOTAL:$minutes min.$ci"
   }

set hoursC [format %2.0f [expr [format %2.0f $mom_cutting_time] / 60]]
set minutesC [format %2.0f [expr $mom_cutting_time - 60 * $hours]]

if { $hours > 0} {
MOM_output_literal "$co TIEMPO CORTE:$hoursC Hrs.$minutesC min.$ci"
   } else {
MOM_output_literal "$co TIEMPO CORTE:$minutesC min.$ci"
   }
}
...
MOM_output_literal "(CGA Compañía General de Aceros S.A. - MAZAK_VTC250 3ax)"
...
set mom_kin_retract_distance            255.0
set mom_kin_reengage_distance           25.0
...
#=============================================================
proc PB_CMD_inspect_cycle { } {
#=============================================================
global mom_cycle_feed_to_pos mom_cycle_spindle_axis z z2 c mom_sys_cycle_off mom_sys_rapid_code
global mom_cycle_rapid_to_pos mom_cycle_rapid_to_pos mom_pos
# global zr
set c [expr $c+1]
set z $mom_cycle_feed_to_pos($mom_cycle_spindle_axis)
set z [expr abs($z)]
set xf [format %2.3f $mom_cycle_rapid_to_pos(0)]
set yf [format %2.3f $mom_cycle_rapid_to_pos(1)]
# set zrf [format %2.0f $zr]

if { $c > 1 } {
    set d [expr abs($z2-$z)]
        if { $d > 0 } {
            MOM_output_literal "G$mom_sys_cycle_off"
##          MOM_output_literal "G$mom_sys_rapid_code Z$zrf"
            MOM_output_literal "G$mom_sys_rapid_code X$xf Y$yf"
            }
   }
...
#
  ...
  set date1 [string toupper $mom_date]
    MOM_output_text "(UNIVERSIDAD NACIONAL DE COLOMBIA)"
    MOM_output_text "(POSTPROCESADOR CNC HARTFORD CONTROL MITSUBISHI)"
    MOM_output_text "(DEPARTAMENTO DE INGENIERIA MECANICA Y MECATRONICA)"
    MOM_output_text "(LABORATORIO DE MAQUINAS HERRAMIENTAS)"

if  {![info exists mom_group_name]} {set mom_group_name ""} {
    MOM_output_literal "(PROGRAMA: $mom_group_name)" }

    MOM_output_text "(POST PROCESADO EL: $date1)"
...
MOM_output_text "( OPERATION :$mom_operation_name)"
}


#=============================================================
proc PB_CMD_oper_info_1 { } {
#=============================================================
global mom_operation_name
...
#=============================================================
proc PB_CMD_operation_information { } {
#=============================================================
...
MOM_output_literal "$co OPERATION NAME: $mom_path_name - T$mom_tool_number : $mom_tool_name  $ci"
}
...
#=============================================================
# Only 5-axis dual-head mill posts will be affected by this
# custom command.
#
# This command revert kinematic parameters for 5-axis dual-head
# mill posts to maintain compatibility and to allow the posts
# to run in UG/Post prior to NX3.
#
# The conversion assumes the constructs of the 4th & 5th addresses
# are identical except their leaders. Only the address leaders are
# swapped here. If more attributes need to be exchanged,
# a definitin file fragment containing new constructs
# of the addresses can be written out to a temporary text file
# and then loaded back into the post with command :
#
#   MOM_load_definition_file <file name>
#
#-------------------------------------------------------------
...
#=============================================================
#  Import this command to the post. It will be executed, when present,
#  automatically in the MOM_set_csys event handler.
#
}
...
#
 ...
 ## Determine if the tool axis is along a principal direction
  ...
  VEC3_unitize mom_tool_axis vec
  if { ![EQ_is_equal [expr abs($vec(0))] 1.0] && ![EQ_is_equal [expr abs($vec(1))] 1.0] && ![EQ_is_equal [expr abs($vec(2))] 1.0] } {
    set is_principal 0
  }
...
#=============================================================
global mom_sys_control_in mom_sys_control_out co ci
...
# This procedure is executed at the begining of each path in the task.
    ...
    global mom_tool_number
        ...
        MOM_output_literal "(OPERACION=$mom_path_name  HERRAMIENTA=$mom_tool_name)"
        MOM_output_literal "(T=$mom_tool_number D=[format "%.2f" $mom_tool_diameter] R=[format "%.2f" $mom_tool_corner1_radius] LE=[format "%.2f" $mom_tool_flute_length] LT=[format "%.2f" $mom_tool_length])"
...
proc RAPID_SET { } {
  ...
  if { [info exists mom_from_pos($mom_cycle_spindle_axis)] == 0 &&  [info exists  mom_sys_home_pos($mom_cycle_spindle_axis)] } {

    set mom_from_pos(0) $mom_sys_home_pos(0)
    set mom_from_pos(1) $mom_sys_home_pos(1)
    set mom_from_pos(2) $mom_sys_home_pos(2)

  } elseif { [info exists  mom_sys_home_pos($mom_cycle_spindle_axis)] == 0 &&  [info exists mom_from_pos($mom_cycle_spindle_axis)] } {

    set mom_sys_home_pos(0) $mom_from_pos(0)
    set mom_sys_home_pos(1) $mom_from_pos(1)
    set mom_sys_home_pos(2) $mom_from_pos(2)

  } elseif { [info exists  mom_sys_home_pos($mom_cycle_spindle_axis)] == 0 &&  [info exists mom_from_pos($mom_cycle_spindle_axis)] == 0 } {
...
return
 ...
 #<02-18-08 gsl> Reverse workplane change direction per tool axis
  global mom_tool_axis
  switch $mom_cycle_spindle_axis {
    0 {
      if [EQ_is_equal $mom_tool_axis(0) -1.0] {
        set going_lower [expr abs($going_lower - 1)]
      }
    }
    1 {
      if [EQ_is_equal $mom_tool_axis(1) -1.0] {
        set going_lower [expr abs($going_lower - 1)]
      }
    }
  }
  ...
  if { [info exists mom_sys_work_plane_change] == 0 } {
    set mom_sys_work_plane_change 1
  }
