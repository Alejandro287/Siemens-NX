...
#
#  Created by Aio @ Monday, December 26, 2016 9:42:37 PM SA Pacific Standard Time
...
#=============================================================
  ...
  global mom_motion_event
   ...
   set rapid_traverse_blk {G_mode G_motion G_adjust X Y Z H}
...
#=============================================================
   ...
   MOM_do_template rewind_stop_code
   MOM_output_literal "(SMC5 HARTFORD UNAL)"
...
# set default value for flag variable dpp_ge(cycle_plane_change)
  ...
  if {$dpp_ge(toolpath_axis_num)=="3" && $dpp_ge(sys_coord_rotation_output_type)=="SWIVELING"} {
     ...
     if {[string compare "NONE" $dpp_ge(coord_rot)]} {
        ...
        # if it's not auto3d condition, but C axis rotate, change the solution to C=0, mom_pos(i)=mom_mcs_goto(i)
        if {![EQ_is_equal $mom_out_angle_pos(0) 0] || ![EQ_is_equal $mom_out_angle_pos(1) 0]} {
           MOM_output_literal "G49"
           MOM_output_literal "G69"
           MOM_force Once G_motion G_adjust tap_string X Y Z H F R dwell cycle_step
           ...
           set mom_pos(3) 0
           set mom_pos(4) 0
           VMOV 3 mom_mcs_goto mom_cycle_feed_to_pos
           set mom_cycle_feed_to_pos(2) [expr $mom_cycle_feed_to_pos(2)+$mom_cycle_feed_to]
           MOM_reload_variable -a mom_out_angle_pos
     ...
     if {[string compare "LOCAL" $dpp_ge(coord_rot)]} {
        ...
        if {![EQ_is_equal $mom_out_angle_pos(0) $mom_prev_out_angle_pos(0)] || \
           ...
           if {[string compare "NONE" $dpp_ge(coord_rot)]} {
              ...
              set mom_out_angle_pos(1) 0
              VMOV 3 mom_mcs_goto mom_pos
              # set mom_pos(3) and mom_pos(4) to reload mom_prev_out_angle_pos
              set mom_pos(3) 0
              set mom_pos(4) 0
              MOM_reload_variable -a mom_out_angle_pos
              MOM_reload_variable -a mom_pos
...
# 07-23-2012 ljw - Initial version
   ...
   if {$mom_cycle_tool_axis_change ==0 && $mom_cycle_clearance_plane_change == 1 && $dpp_ge(cycle_hole_counter)!=0} {
      MOM_force Once G_motion
...
#<09-09-2015 ljt> - Lock mom_prev_pos, raise a warning when linearization iteration failed.
   ...
   # <09-Sep-2015 ljt> Make sure mom_prev_pos is locked, even if mom_prev_pos is locked, then lock again is fine.
   # If mom_pos is reloaded when someone calls MOM_POST_convert_point in core code, he may get wrong result.
